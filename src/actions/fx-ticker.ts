...
import { renderFxButton, renderError, renderLoading } from '../render';
...
@action({ UUID: 'com.alex.market-monitor.fx-ticker' })
export class FxTickerAction extends SingletonAction<Settings> {
  ...
  // ─── Key press — open Yahoo Finance page for the pair ────────────────────
  ...
  private async refresh(state: InstanceState): Promise<void> {
    void state.action.setImage(renderLoading(state.symbol.replace(/=X$/i, '')));
    void state.action.setTitle('');

    try {
      const quote = await fetchQuote(state.symbol);
      void state.action.setImage(renderFxButton(quote.symbol, quote.price));
      void state.action.setTitle('');
    } catch {
      void state.action.setImage(renderError(state.symbol.replace(/=X$/i, '')));
      void state.action.setTitle('');
    }
  }
